...
export async function GET(request) {
  try {
    ...
    return NextResponse.json({
      aliases,
      strategy: normalizeMitmAliasStrategy(settings?.mitmAliasStrategy),
    });
  } catch (error) {
...
export async function PUT(request) {
  try {
    ...
    const filtered = normalizeMitmAliasMappings(mappings);
    const normalizedStrategy = normalizeMitmAliasStrategy(strategy);
