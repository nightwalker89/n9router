...
import { STREAM_FIRST_CHUNK_TIMEOUT_MS, HTTP_STATUS, DEFAULT_KIRO_RETRY_COUNT } from "../config/runtimeConfig.js";
...
export class KiroExecutor extends BaseExecutor {
  ...
  // Resolve max retry count: per-account → global setting → default. Applied to
  // every in-place-retryable status (429/500/502/503/504) plus 400, whose actual
  // retry eligibility is decided per-body in computeRetryDelay (see below).
  getRetryConfig(credentials) {
    const attempts = credentials?.kiroRetryCount ?? DEFAULT_KIRO_RETRY_COUNT;
    const delayMs = 2000;
    return {
      ...this.config.retry,
      [HTTP_STATUS.RATE_LIMITED]: { attempts, delayMs },
      500: { attempts, delayMs },
      502: { attempts, delayMs },
      503: { attempts, delayMs },
      504: { attempts, delayMs },
      400: { attempts, delayMs },
    };
  }

  // 400 is normally a terminal validation error, but Kiro occasionally returns
  // "improperly formed request" for what is actually a transient upstream hiccup.
  // getRetryConfig() enables in-place retry for status 400 unconditionally; this
  // hook vetoes it (no attempt consumed) unless the body matches a known transient
  // pattern, so genuine validation errors still fail fast. Non-400 statuses get
  // the same exponential-backoff-with-jitter curve as the pre-merge implementation.
  async computeRetryDelay(response, attempt) {
    if (response.status === HTTP_STATUS.BAD_REQUEST) {
      let bodyText = "";
      try { bodyText = await response.clone().text(); } catch { /* ignore */ }
      if (!this._isRetryable400(bodyText)) return false;
    }
    return this._computeBackoff(attempt);
  }
  ...
  _computeBackoff(attempt) {
    return Math.floor(Math.random() * Math.min(KIRO_BACKOFF_CAP_MS, KIRO_BACKOFF_BASE_MS * 2 ** (attempt - 1))) + KIRO_BACKOFF_BASE_MS / 2;
  }
  ...
  async execute(args) {
    ...
    // Retries in getRetryConfig()/computeRetryDelay() are exhausted by the time
    // execute() returns. A still-400 transient response at this point means every
    // attempt failed the same way — remap to 429 so callers (e.g. Claude Code CLI)
    // that only auto-retry 429/503 treat it as retryable instead of terminal.
    if (result?.response && !result.response.ok && result.response.status === HTTP_STATUS.BAD_REQUEST) {
      const bodyText = await result.response.clone().text().catch(() => "");
      if (this._isRetryable400(bodyText)) {
        args.log?.debug?.("RETRY", "400 → 429 remap (transient Kiro error)");
        result.response = new Response(bodyText, {
