...
describe("Responses abort terminal synthesis", () => {
  it("emits response.failed + [DONE] when upstream errors (abort/stall)", async () => {
    ...
    const out = createDisconnectAwareStream(
      ...
      makeController(),
      true, // streamWatchdogEnabled (fork param; onAbortTerminal moved to 4th in the merge union)
  ...
  it("does not synthesize terminal for non-Responses streams (callback null)", async () => {
    ...
    const out = createDisconnectAwareStream(
      { readable: upstream, writable: { getWriter: () => ({ abort: () => Promise.resolve() }) } },
      makeController(),
      false, // streamWatchdogEnabled OFF: isolates the "no terminal callback" path from the fork's watchdog [DONE] sentinel
      null   // onAbortTerminal: none → no synthesized terminal for non-Responses streams
    );
