...
describe("DB SQLite layer — public API parity", () => {
  ...
  it("compat db.json: syncs settings and provider connections for MITM mode B", async () => {
    await sqliteDb.updateSettings({ tokenSwapEnabled: true, tokenSwapStrategy: "sticky" });
    const c = await sqliteDb.createProviderConnection({
    ...
    const legacy = JSON.parse(fs.readFileSync(path.join(tempDir, "db.json"), "utf8"));
    expect(legacy.settings.tokenSwapEnabled).toBe(true);
    expect(legacy.settings.tokenSwapStrategy).toBe("sticky");
    expect(legacy.providerConnections.find((item) => item.id === c.id)).toMatchObject({
      provider: "antigravity",
      email: "ag@example.com",
      accessToken: "ag-token",
      refreshToken: "ag-refresh",
