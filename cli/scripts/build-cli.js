...
function buildCliPackage() {
  ...
  // Step 3b: Ensure external runtime modules are present in the standalone
  // bundle. Persistence remains lowdb/db.json; do not add SQLite drivers here.
  console.log("3️⃣ b Bundling external runtime modules...");
  function ensureModuleInBundle(pkg) {
    ...
    if (!src) {
      console.warn(
        `⚠️  ${pkg} not found locally — the standalone bundle may be incomplete`,
      );
      return;
    }
