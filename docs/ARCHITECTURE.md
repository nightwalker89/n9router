...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.n9router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.n9router/usage.json`, `~/.n9router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.n9router/db.json`)
- usage stats: `~/.n9router/usage.json`
- request log lines: `~/.n9router/log.txt`
...
1. `usageDb` currently stores under `~/.n9router` and does not follow `DATA_DIR`.
